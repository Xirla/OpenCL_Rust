...
fn main() {
    // get platforms
    let platforms = opencl3::platform::get_platforms().expect("Failed to get platforms.");
    ...
    // create context
    let context =
        opencl3::context::Context::from_device(&device).expect("Failed to create context.");

    let source = std::fs::read_to_string("./src/vadd.cl").unwrap();

    // create program
    let mut program = opencl3::program::Program::create_from_source(&context, &source)
    ...
    // create kernel
    let kernel =
        opencl3::kernel::Kernel::create(&program, "vadd").expect("Failed to create kernel.");

    // create command queue
    let command_queue = opencl3::command_queue::CommandQueue::create(
        &context,
        device.id(),
    ...
    // create buffer
    let mut a_buffer = opencl3::memory::Buffer::<opencl3::types::cl_int>::create(
        &context,
    ...
    let mut b_buffer = opencl3::memory::Buffer::<opencl3::types::cl_int>::create(
        &context,
        opencl3::memory::CL_MEM_READ_ONLY,
        10,
        std::ptr::null_mut(),
    ...
    let mut c_buffer = opencl3::memory::Buffer::<opencl3::types::cl_int>::create(
        &context,
    ...
    // write buffer
    command_queue
        .enqueue_write_buffer(&mut a_buffer, opencl3::types::CL_TRUE, 0, &a[..], &[])
        .expect("Failed to write buffer a.");
    command_queue
        .enqueue_write_buffer(&mut b_buffer, opencl3::types::CL_TRUE, 0, &b[..], &[])
        .expect("Failed to write buffer b.");
    ...
    command_queue
        ...
        .expect("Failed to read buffer c.");
